...
class UrgencyScreen extends ConsumerStatefulWidget {
  const UrgencyScreen({super.key});

  @override
  ConsumerState<UrgencyScreen> createState() => _UrgencyScreenState();
}

class _UrgencyScreenState extends ConsumerState<UrgencyScreen> {
    ...
    if (phone == null || phone.isEmpty) {
      showDialog(
        ...
        builder: (context) => AlertDialog(
          ...
          content: const Text('Você ainda não cadastrou um contato de confiança.'),
          actions: [
            TextButton(
            ...
            ElevatedButton(
              onPressed: () {
                ...
                context.push(ForjaRoutes.supportContact);
              },
                        ...
                        children: [
                          ...
                          SizedBox(
                            ...
                            child: ElevatedButton.icon(
                              ...
                              label: const Text('LIGAR AGORA', style: TextStyle(fontWeight: FontWeight.bold)),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(subtitle, style: const TextStyle(color: ForjaColors.textSecondary)),
                ],
              ),
            ),
            const Icon(Icons.chevron_right_rounded, color: ForjaColors.textSecondary),
          ],
        ),
      ),
    );
...
class _GuidedBreathingState extends State<_GuidedBreathing> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Center(
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          Text('Ciclo $_cycle de 3', style: const TextStyle(color: ForjaColors.textSecondary)),
            ...
            builder: (context, child) {
              return Container(
                ...
                decoration: BoxDecoration(
                  ...
                  color: ForjaColors.ember.withValues(alpha: 0.1 + (0.2 * _controller.value)),
                ...
                child: Center(
                  child: Text(
                    _text,
                    style: const TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: ForjaColors.ember),
                  ),
                ),
              );
            },
...
class _EmergencyTimerState extends State<_EmergencyTimer> with SingleTickerProviderStateMixin {
    ...
    if (_isCompleted) {
      return Center(
        child: Column(
          ...
          children: [
            ScaleTransition(
              scale: CurvedAnimation(parent: _confettiController, curve: Curves.elasticOut),
              child: const Icon(Icons.emoji_events_rounded, color: ForjaColors.ember, size: 80),
            ),
            const SizedBox(height: 24),
            Text('VITÓRIA!', style: text.displayMedium?.copyWith(color: ForjaColors.ember)),
